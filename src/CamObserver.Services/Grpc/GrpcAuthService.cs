...
namespace CamObserver.Services.Grpc
{
    public class GrpcAuthService : IAuth
    {
        ...
        public Task<AuthenticatedUserModel> AuthenticateWithApiKey(AuthenticationModel data, CallContext context = default)
        {
            ...
            var res = new AuthenticatedUserModel() { AccessToken = CamObserver.Tools.RandomGenerator.StaticGenerator.RandomNumber(10), ExpiredDate = DateHelper.GetLocalTimeNow().AddDays(1), TokenType = "random", Username = "admin" };
            if (Gagal)
            {
        ...
        public Task<AuthenticatedUserModel> AuthenticateWithUsername(AuthenticationUserModel data, CallContext context = default)
        {
            ...
            var res = new AuthenticatedUserModel() { AccessToken = CamObserver.Tools.RandomGenerator.StaticGenerator.RandomNumber(10), ExpiredDate = DateHelper.GetLocalTimeNow().AddDays(1), TokenType = "random", Username = Nama };
            if (!isAuthenticate)
            {
                res.Username = String.Empty;
                res.ExpiredDate = DateHelper.GetLocalTimeNow().AddDays(-1);
